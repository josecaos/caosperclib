...
CaosKick {
	...
	*robot {|att= 0.01, rel= 0.5, modFreq= 1, modbw= 0.1, freq1= 60, freq2= 66, lowcutfreq= 50,  gate= 0, amp1= 0.5, amp2= 0.5,doneaction=2, impulsefreq=1, tempo=1 |
		var kick,env;
   	     		kick=RHPF.ar(
				   		LFTri.ar(Pulse.ar(tempo*modFreq,modbw,freq1,freq2),0,amp1),
        		             lowcutfreq,0.5)+
								SinOsc.ar(Mix(60,82,280),0,amp2)+
        		               		LFTri.ar(Pulse.ar(tempo*modFreq,modbw,freq1,freq2),0,amp2);
        		kick=CompanderD.ar(kick,0.6,0.59,0.8);
		env=EnvGen.ar(Env.perc(att,rel),Impulse.kr( impulsefreq * tempo ),doneAction:0);
    		^Pan2.ar(kick*env,[-1,0.98]);
		}
	}
