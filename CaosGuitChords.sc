...
CaosGuitChords {
		*ar{|chord = 'M', att = 0.1, rel = 1, note = 57, cutf = 1200, rq = 0.5, pan = 0, gate = 0, amp = 0.5|
		...
		//
			if(chord!=chords || chord == nil,
				{10.do{"ERR: Use 'M', 'm', 'M7', 'm7', 'Mmaj7', 'mmaj7', 'M9',  'M9m', 'm9' or 'm9m' only as first CaosChord.ar argument".postln}}//
			);
			...
			octfifth=notes[4];
			oct=notes[5];
			sint=(LFTri.ar(notes.midicps,0,amp/2.3)+
					Pulse.ar(fifth.midicps,0.1,amp/2.4)+
					Pulse.ar(seventh.midicps,0.1,amp/2.3)+
					Pulse.ar(third.midicps,0.3,amp/2.4)+
					Pulse.ar(octfifth.midicps,0.2,amp/2.6)+
					Pulse.ar(oct.midicps,0.1,amp/2.6);
		);
